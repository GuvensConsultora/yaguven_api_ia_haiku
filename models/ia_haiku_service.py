...
EXTRACTION_SYSTEM = (
    ...
    "'Serial' / 'N° Serie'), UNO por unidad. Cada serie es un identificador "
    "ÚNICO de la unidad física, casi siempre ALFANUMÉRICO (ej. FCH13039A, "
    "ICH01467F): es DISTINTO del 'codigo' del producto (que se REPITE entre "
    "renglones del mismo producto) y DISTINTO del CAI / código de autorización. "
    "NO confundas columnas: el orden típico es CAI, Medida, Descripción, "
    "Codigo, Serie, Cantidad — la serie va en su propia columna. Transcribí "
    "cada serie CARÁCTER POR CARÁCTER, sin adivinar, sin normalizar, sin "
    "agregar ni quitar dígitos o letras; si un carácter es dudoso poné el que "
    "más se parece (no inventes una serie entera ni la completes con ceros). "
    "Leé los renglones EN ORDEN, uno por uno, sin saltear ni reordenar. Si la "
    "cantidad es 3 y hay 3 seriales, listá los 3; si el renglón no trae serie, "
    "dejá la lista vacía. "
    ...
    "'descuento' es el % de descuento COMERCIAL/bonificación si el PDF lo expresa "
    "en porcentaje; si el descuento figura como un monto en pesos, dejá 'descuento' "
    "en 0 y asegurate de completar 'importe' (el sistema calcula el % desde importe). "
    "IMPORTANTE: 'descuento' NUNCA es el % de IVA ni una alícuota impositiva. Si la "
    "única columna de '%' del renglón es el IVA (21, 10,5, 27, etc.), NO la pongas en "
    "'descuento' — dejá 'descuento' en 0. El IVA va por producto, no acá. "
